    ...
    <>
      ...
      >
        {mistakes.map((m) => (
          <li key={m.id} className={cn(viewMode === "grid" && "h-full")}>
            {viewMode === "grid" ? (
              <div className="flex h-full w-full flex-col overflow-hidden rounded-lg border border-border-default bg-surface-elevated text-left shadow-sm">
                <button
                  className="flex w-full flex-1 flex-col items-start p-4 pb-3 text-left transition-colors hover:bg-black/[0.04] dark:hover:bg-surface-elevated/[0.04]"
                  onClick={() => m.problem && setDetailTarget(m.problem)}
                >
                  <p className="mb-2 line-clamp-4 whitespace-pre-wrap text-sm font-semibold leading-5 text-text-primary">
                    {m.content}
                  </p>
                  <p className="line-clamp-2 text-xs leading-5 text-text-secondary">{m.problem?.question}</p>
                ...
                <div className="mt-auto flex items-center justify-between gap-2 border-t border-border-subtle px-4 py-3">
                  ...
                  <div className="flex shrink-0 items-center gap-1 text-xs text-text-tertiary">
                    <span className="mr-1 text-[11px] text-text-tertiary">{formatDate(m.createdAt)}</span>
                    <button
                      type="button"
                      onClick={(e) => startEdit(e, m)}
                      className="rounded-lg px-2 py-1.5 text-text-secondary transition-colors hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-950/50"
                    >
                      {t("common.edit")}
                    </button>
                    <button
                      ...
                      className="rounded-lg px-2 py-1.5 text-text-secondary transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/50"
                    >
                      {t("mistakes.delete")}
                    </button>
                  </div>
              ...
              <div className="grid gap-3 rounded-lg border border-border-default bg-surface-elevated px-3 py-3 shadow-sm sm:grid-cols-[minmax(9rem,1fr)_minmax(12rem,1.5fr)_7rem_8rem_auto] sm:items-center">
                <button
                  type="button"
                  onClick={() => m.problem && setDetailTarget(m.problem)}
                  className="min-w-0 text-left"
                >
                  <p className="truncate text-sm font-medium text-text-primary">{m.problem?.question}</p>
                ...
                <button
                  ...
                  className="min-w-0 text-left"
                >
                  <p className="line-clamp-2 whitespace-pre-wrap text-sm text-text-primary">{m.content}</p>
                </button>
                <div className="hidden sm:block">
                  <CategoryBadge category={m.problem?.category} />
                </div>
                <span className="text-xs text-text-tertiary">{formatDate(m.createdAt)}</span>
      ...
      {editingRecord && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="mx-4 w-full max-w-lg rounded-lg bg-surface-overlay p-6 shadow-popover">
            ...
            <textarea
              ...
              className="min-h-32 w-full resize-y rounded-lg border border-border-default bg-surface-elevated px-3 py-2 text-sm outline-none focus:border-emerald-500"
            />
